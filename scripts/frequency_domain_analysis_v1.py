...
parser.add_argument("--debug", action="store_true", help="Show full output including MNE logs")
...
# suppress MNE output unless debug
import mne
...
label_pbar = tqdm(label_segments.items(), desc="Plotting labels", unit="label")

for label, channel_data in label_pbar:
    label_pbar.set_postfix(label=label)
    ...
    # ── trim ─────────────────────────────────────────────────────────────────
    plot_magnitude(mag_trim,   label, trim_dir / label, "Trim to Minimum",
                   "All segments trimmed to the shortest segment length", n_patients, label_segment_counts)
    plot_phase(phase_trim,     label, trim_dir / label, "Trim to Minimum",
               "All segments trimmed to the shortest segment length", n_patients, label_segment_counts)
    plot_frequency_response(freq_trim, label, trim_dir / label, "Trim to Minimum",
                            "All segments trimmed to the shortest segment length", n_patients, label_segment_counts)
    ...
    # ── pad ──────────────────────────────────────────────────────────────────
    plot_magnitude(mag_pad,    label, pad_dir / label, "Zero Pad to Maximum",
                   "Shorter segments zero padded to the longest segment length", n_patients, label_segment_counts)
    plot_phase(phase_pad,      label, pad_dir / label, "Zero Pad to Maximum",
               "Shorter segments zero padded to the longest segment length", n_patients, label_segment_counts)
    plot_frequency_response(freq_pad, label, pad_dir / label, "Zero Pad to Maximum",
                            "Shorter segments zero padded to the longest segment length", n_patients, label_segment_counts)
    ...
    # ── interpolate ───────────────────────────────────────────────────────────
    plot_magnitude(mag_interp,   label, interpolate_dir / label, "Interpolate to Fixed Length",
                   f"All segments resampled to {FIXED_LEN} points via cubic interpolation", n_patients, label_segment_counts)
    plot_phase(phase_interp,     label, interpolate_dir / label, "Interpolate to Fixed Length",
               f"All segments resampled to {FIXED_LEN} points via cubic interpolation", n_patients, label_segment_counts)
    plot_frequency_response(freq_interp, label, interpolate_dir / label, "Interpolate to Fixed Length",
                            f"All segments resampled to {FIXED_LEN} points via cubic interpolation", n_patients, label_segment_counts)
...
tqdm.write(f"all CSVs saved to:   {analysis_dir}")